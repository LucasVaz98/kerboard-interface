...
uint16_t HIDUniversal::GetHidClassDescrLen(uint8_t type, uint8_t num) {
  if (DEBUG)
    Serial.println("hiduniversal.cpp -> HIDUniversal.GetHidClassDescrLen");
...
void HIDUniversal::Initialize() {
  ...
  if (DEBUG)
    Serial.println("hiduniversal.cpp -> HIDUniversal.Initialize");
...
bool HIDUniversal::SetReportParser(uint8_t id, HIDReportParser *prs) {
  ...
  if (DEBUG)
    Serial.println("hiduniversal.cpp -> HIDUniversal.SetReportParser");
