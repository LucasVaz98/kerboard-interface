...
uint8_t HIDBoot<BOOT_PROTOCOL>::Init(uint8_t parent, uint8_t port, bool lowspeed) {
        ...
        if(DEBUG)
        {
          Serial.println("BM Init");
          Serial.println("totalEndpoints:" + (uint8_t) (totalEndpoints(BOOT_PROTOCOL)));
          Serial.println("epMUL:" + epMUL(BOOT_PROTOCOL));
        }
